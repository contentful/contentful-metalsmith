 ...
 */
function getCommonContentForSpace (options, entries) {
  if (!options.common) {
    return entries
  }
  ...
  for (let id in options.common) {
    commonQueries.push(client.getEntries(util.getEntriesQuery(options.common[id])))
    commonIds.push(id)
  }

  // First, execute all common queries.
  return Promise.all(commonQueries).then(commonContent => {
    // Finally, deliver the entries and common queries for further processing.
    return new Promise((resolve, reject) => {
      // Store the common entries as a keyed object using commonIds for keys.
      const commonsObj = commonIds.reduce((prev, curr, index) => {
        prev[curr] = commonContent[index]
        return prev
      }, {})

      // Assign common entries to each entry.
...
function processFile (file, options) {
  ...
  return client.getEntries(query)
    ...
    .then(entries => getCommonContentForSpace(options, entries))
}
