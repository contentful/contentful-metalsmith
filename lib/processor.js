...
function getCommonContentForSpace (entries, options) {
  ...
  const client = getContentfulClient(options.access_token, options.space_id, options.host, option.environment)
...
function processFile (file, options) {
  ...
  return client.getEntries(query)
    .catch(error => (decorateAPIError(error, file, query)))
 ...
 *
 */
function decorateAPIError (error, file, client) {
  throw new Error(
